...
static HE_S32 debug_comm_init(HE_VOID)
{
	...
	{        
	    ...
	    ios_dbg.c_cflag = B115200 | CS8 | CLOCAL | CREAD;
		...
		cfsetispeed(&ios_dbg, B115200);
		cfsetospeed(&ios_dbg, B115200);

        tcsetattr( debug_comm_context.fd_com, TCSANOW, &ios_dbg );
		//tcflush(debug_comm_context.fd_com,TCIOFLUSH);
		write_log("debug_comm_init:set debug comm0 B115200");
    }
...
static HE_S32 BD1_comm_init(HE_VOID)
{
	...
	BD1_comm_context.fd_com = open("/dev/ttySAC2",O_RDWR);
	...
	write_log("BD1_comm_init:set comm2 B115200");
	return HE_SUCCESS;
}
